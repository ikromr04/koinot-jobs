        ...
        @foreach ($vacancies as $vacancy)
            <li class="flex">
                <a class="flex flex-col text-inherit no-underline py-8 px-6 border rounded-[20px] border-[#C4C9CE]" href="{{ route('pages.vacancy', $vacancy->id) }}">
                    <div class="vacancy-card__title !text-[20px] font-bold">{{ strip_tags($vacancy->translation?->title) }}</div>
                    ...
                    <div class="flex justify-between items-center mt-auto">
                        ...
                        <div class="flex items-center gap-2">
                            @if ($vacancy->company?->translation?->logo)
                                <img
                                    class="w-[80px] h-6 object-contain"
                                    src="{{ asset('/storage/' . $vacancy->company?->translation?->logo) }}"
                                    width="80"
                                    height="24"
                                    alt="{{ $vacancy->company?->translation?->title }}"
                                >
                            @endif
                        </div>
                    </div>
